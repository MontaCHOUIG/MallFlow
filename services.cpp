...
QSqlQueryModel* Services::afficher()
{
    ...
    QSqlQuery query;
    query.exec("SELECT ID_Service, Nom_Service, Type_Service, Date_Service, Etat FROM Services");
...
QSqlQueryModel* Services::trierParNom()
{
    QSqlQueryModel *model = new QSqlQueryModel();
    QSqlQuery query;
    query.exec("SELECT * FROM Services ORDER BY Nom_Service ASC");
    model->setQuery(std::move(query)); // Utilisation de std::move
    return model;
}
...
QSqlQueryModel* Services::trierParType()
{
    QSqlQueryModel *model = new QSqlQueryModel();
    QSqlQuery query;
    query.exec("SELECT * FROM Services ORDER BY Type_Service ASC");
    model->setQuery(std::move(query)); // Utilisation de std::move
    return model;
}
...
QSqlQueryModel* Services::rechercherParType(const QString &type)
{
    QSqlQueryModel *model = new QSqlQueryModel();
    QSqlQuery query;
    query.prepare("SELECT * FROM Services WHERE Type_Service LIKE :type");
